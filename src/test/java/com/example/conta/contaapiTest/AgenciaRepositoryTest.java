...
public class AgenciaRepositoryTest {
	...
	@Test
	public void deveBuscarAgenciaPorNome() {
		Cidade cidade = cidadeRepository.save(new Cidade("Maringá", Estado.PR));
		Agencia agencia = new Agencia("6544", "2", cidade);
		
		agenciaRepository.save(agencia);
		
		Agencia agenciaBuscadaPorNome = agenciaRepository.findByNumero("6544");
		
		Assert.assertNotNull(agenciaBuscadaPorNome.getId());
		Assert.assertEquals(agencia.getNumero(), agenciaBuscadaPorNome.getNumero());
		Assert.assertEquals(agencia.getDigito(), agenciaBuscadaPorNome.getDigito());
	}
